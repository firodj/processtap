...
class Elf(File):
  def __init__(self, name = None, buf = None):
    ...
    if ehdr.e_type == ET_REL:
        self.setType((self.getType()[0], "REL"))
        parse_et_rel(self, name, buf, self.ei_class)
        return
    elif ehdr.e_type == ET_DYN:
        self.setType((self.getType()[0], "DYN"))
        # shared object file
        # return
        pass
    elif ehdr.e_type == ET_EXEC:
        self.setType((self.getType()[0], "EXEC"))
    ...
    for phno in range(ehdr.e_phnum):
      ...
      if ph.p_type == PT_LOAD:
        # loadable segment
        for s in shdr:
          if not s.getName().startswith(".gnu.warning") and s.getLowAddr() >= ph.p_vaddr and s.getHighAddr() <= ph.p_vaddr + ph.p_memsz:
    ...
    if syms:
      ...
      for x in ssyms:
        ...
        s = Symbol(name = name, size = x.st_size,
                   address = x.st_value, symtype = typ,
                   module = self)
        self.addSymbol(s)
    ...
    if dynamic:
      ...
      for dsn in range(len(dynsyms)):
        ...
        s = Symbol(name = name, size = d.st_size, address = d.st_value, symtype = typ, module = self)
        self.addSymbol(s)
      ...
      for rr in rels:
        ...
        for r in range(rr.sh_size / rr.sh_entsize):
          ...
          s = Symbol(name = name, size = size, address = s.r_offset, symtype = typ, module = self)
          self.addSymbol(s)
